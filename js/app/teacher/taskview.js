var TaskAdd = new J.Class({
    ...
    initAvalon: function() {
        ...
        this.vm = avalon.define({
            ...
            back: function() {
                if (self.classId) {
                    J.goPage("editlesson.html", { courseid: self.courseId, classid: self.classId });
                } else {
                    J.goPage("task.html");
                }
    ...
    addMedia: function(audioList) {
        for (var i = 0; i < audioList.length; i++) {
            ...
            this.vm.task.segmentModuleList[0].contentList.push(obj);
    ...
    httpAddTask: function() {
        ...
        for (var t = 0; t < list.length; t++) {
            ...
            list[t].contentList = contentList.concat(tempList);
        }
        if (flag) {
            return;
        }
    ...
    httpReadTask: function() {
        var self = this;
        var path = "/segment/read";
        if (self.classId) {
            path = "/course/segment/read";
        }
        J.ajax({ url: path, type: 'POST' }, { segmentId: this.segmentId }, function(data) {
